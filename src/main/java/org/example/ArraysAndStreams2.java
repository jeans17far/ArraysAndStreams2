...
public class ArraysAndStreams2 {
    public static void main(String[] args) {
        ...
        System.out.printf("strings that start with a vowel: %s%n",
                Arrays.stream(strings)
                        .filter(s -> s.matches("(?i)^[aeiou].*")) // regex to check for starting vowels
                        .collect(Collectors.toList()));

        // concatenate all strings into a single string separated by a comma
        String concatenatedString = Arrays.stream(strings)
                .collect(Collectors.joining(", "));
        System.out.printf("Concatenated string: %s%n", concatenatedString);

        // count the number of strings that contain more than 5 characters
